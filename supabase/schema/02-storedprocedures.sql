...
-- - Resuelve conflictos con la restriccion UNIQUE de email antes del
--   upsert (ver pasos A y B), que de otro modo abortaban la importacion
--   completa con error 23505 sobre students_email_key.
...
BEGIN
    ...
    -- students.email es UNIQUE, pero el upsert solo resuelve conflictos
    -- por carnet. Si el correo entrante pertenece a un carnet que ya no
    -- esta en el padron, es la misma persona con carnet reasignado: se
    -- actualiza el carnet en vez de intentar crear un registro duplicado,
    -- conservando el UUID y con el sus votos, tokens y auditoria.
    WITH migrated AS (
        UPDATE students s
        SET carnet = i.carnet,
            updated_at = NOW()
